...
```{r}
SA1.dist <- read.csv("C:/Users/mattk/OneDrive - UNSW/Data/Census/netdist_sa1_szn.csv")
SA1.dist <- SA1.dist %>% 
...
emp_ai.16 <- emp_ai.16 %>% 
  ...
  mutate(ai = (Destination_Total/DZN_Area) * f_dij)
...
emp_ai.16 <- emp_ai.16 %>% 
  mutate(est_flow = (Origin_Total/SA1_Area) * (Destination_Total/DZN_Area) * Ai * f_dij) %>% 
  mutate(Potential = (Destination_Total/DZN_Area)/f_dij) %>% 
  mutate(Normalised_Potential = normalize(Potential))

CalcRSquared(emp_ai.16$Flow,emp_ai.16$est_flow)

sa1_Access <- aggregate(emp_ai.16$Normalised_Potential, by=list(Origin = emp_ai.16$Origin), FUN = sum)
```
